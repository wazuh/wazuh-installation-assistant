...
for i in "${!users[@]}"; do
    if curl -s -XGET https://127.0.0.1:9200/ -u "${users[i]}":"${pass[i]}" -k -w %{http_code} | grep "401"; then
        exit 1
    fi
done
...
for i in "${!users[@]}"; do
    if curl -s -XGET https://127.0.0.1:9200/ -u "${users[i]}":"${passindexer[i]}" -k -w %{http_code} | grep "401"; then
        exit 1
    fi
done

for i in "${!api_users[@]}"; do
    if curl -s -u "${api_users[i]}":"${passapi[i]}" -w "%{http_code}" -k -X POST "https://127.0.0.1:55000/security/user/authenticate" | grep "401"; then
        exit 1
    fi
done
...
for i in "${!users[@]}"; do
    if curl -s -XGET https://127.0.0.1:9200/ -u "${users[i]}":"${passfile[i]}" -k -w %{http_code} | grep "401"; then
        exit 1
    fi
done
echo '::endgroup::'
...
for i in "${!users[@]}"; do
    if curl -s -XGET https://127.0.0.1:9200/ -u "${users[i]}":"${passindexerf[i]}" -k -w %{http_code} | grep "401"; then
        exit 1
    fi
done

for i in "${!api_users[@]}"; do
    if curl -s -u "${api_users[i]}":"${passapif[i]}" -w "%{http_code}" -k -X POST "https://127.0.0.1:55000/security/user/authenticate" | grep "401"; then
        exit 1
    fi
done
