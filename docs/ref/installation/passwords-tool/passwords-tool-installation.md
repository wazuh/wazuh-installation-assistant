...
```bash
curl -so wazuh-passwords-tool-4.14.8.sh https://packages.wazuh.com/production/4.x/installation-assistant/wazuh-passwords-tool-4.14.8.sh
```

To use `pre-release` packages instead, use the following command:

```bash
curl -so wazuh-passwords-tool-4.14.8.sh https://packages-staging.xdrsiem.wazuh.info/pre-release/4.x/installation-assistant/wazuh-passwords-tool-4.14.8-<STAGE>.sh
```
