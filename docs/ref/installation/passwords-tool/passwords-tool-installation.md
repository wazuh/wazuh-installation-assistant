...
```bash
curl -so wazuh-passwords-tool-5.9.9.sh https://packages.wazuh.com/production/5.x/installation-assistant/wazuh-passwords-tool-5.9.9.sh
```

To use `pre-release` packages instead, use the following command:

```bash
curl -so wazuh-passwords-tool-5.9.9.sh https://packages-staging.xdrsiem.wazuh.info/pre-release/5.x/installation-assistant/wazuh-passwords-tool-5.9.9-<STAGE>.sh
```
