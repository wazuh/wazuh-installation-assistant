...
```bash
curl -so wazuh-passwords-tool-0.0.1.sh https://packages.wazuh.com/production/0.x/installation-assistant/wazuh-passwords-tool-0.0.1.sh
```

To use `pre-release` packages instead, use the following command:

```bash
curl -so wazuh-passwords-tool-0.0.1.sh https://packages-staging.xdrsiem.wazuh.info/pre-release/0.x/installation-assistant/wazuh-passwords-tool-0.0.1-<STAGE>.sh
```
