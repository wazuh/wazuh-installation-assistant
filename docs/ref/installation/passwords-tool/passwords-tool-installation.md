...
```bash
curl -so wazuh-passwords-tool-1.2.3.sh https://packages.wazuh.com/production/1.x/installation-assistant/wazuh-passwords-tool-1.2.3.sh
```

To use `pre-release` packages instead, use the following command:

```bash
curl -so wazuh-passwords-tool-1.2.3.sh https://packages-staging.xdrsiem.wazuh.info/pre-release/1.x/installation-assistant/wazuh-passwords-tool-1.2.3-<STAGE>.sh
```
