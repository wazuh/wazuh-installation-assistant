...
| `-d [pre-release\|local]`, `--development` | Use development repositories. By default it uses the pre-release package repository. If local is specified, it will use a local artifact_urls.yml file located in the same path as the wazuh-install-5.1.0.sh. |
...
```bash
sudo bash wazuh-install-5.1.0.sh --all-in-one
# or use the short form
sudo bash wazuh-install-5.1.0.sh -a
```
...
2. Create the necessary files for installation that will be stored in `wazuh-install-files.tar` with the following command:
    ...
    ```bash
    sudo bash wazuh-install-5.1.0.sh --generate-config-files
    # or use the short form
    sudo bash wazuh-install-5.1.0.sh -g
    ```
...
4. Once you have the `wazuh-install-files.tar` file on the machine where you will install the component, you just need to run the installation command for the desired component:
    ...
    ``` bash
    sudo bash wazuh-install-5.1.0.sh --wazuh-manager
    # or use the short form
    sudo bash wazuh-install-5.1.0.sh -wm
    ```

    4.2 To install the Wazuh Indexer:

    ``` bash
    sudo bash wazuh-install-5.1.0.sh --wazuh-indexer
    # or use the short form
    sudo bash wazuh-install-5.1.0.sh -wi
    ```

    4.3 To install the Wazuh Dashboard:

    ``` bash
    sudo bash wazuh-install-5.1.0.sh --wazuh-dashboard
    # or use the short form
...
1. On a system with Internet access, download the packages of the central components you want to install on the offline system. Note that you also need to have the `wazuh-install-5.1.0.sh` on the system with Internet access.
See the [Installation Assistant Installation](../../installation/installation-assistant/ia-installation.md) section to learn how to download `wazuh-install-5.1.0.sh`.

    To download the packages necessary for offline installation, run the following command:

    ```bash
    sudo bash wazuh-install-5.1.0.sh --download-wazuh <TYPE> --download-arch <ARCH>
    # or use the short form
    sudo bash wazuh-install-5.1.0.sh -dw <TYPE> -da <ARCH>
    ```
...
3. Transfer all files (`wazuh-offline.tar.gz`, `wazuh-install-files.tar` and `wazuh-install-5.1.0.sh`) to the offline system where you want to install Wazuh using your preferred method (USB, SCP, etc).

#### Perform the offline installation

Once you have the `wazuh-offline.tar.gz`, `wazuh-install-files.tar` and `wazuh-install-5.1.0.sh` files on the offline system, the installation is done the same way as a normal Wazuh installation (you can see how it's done in the `AIO Installation` and `Specific Component Installation` sections found above in this document), but by specifying the `-of, --offline-installation` option to the installation command.
For example, to perform an offline AIO installation, the command would be:

```bash
sudo bash wazuh-install-5.1.0.sh --all-in-one --offline-installation
# or use the short form
sudo bash wazuh-install-5.1.0.sh -a -of
```
...
```bash
sudo bash wazuh-install-5.1.0.sh --wazuh-manager --offline-installation
# or use the short form
sudo bash wazuh-install-5.1.0.sh -wm -of
```
...
```bash
sudo bash wazuh-install-5.1.0.sh --all-in-one --development pre-release
# or use the short form
sudo bash wazuh-install-5.1.0.sh -a -d pre-release
```

#### Use development packages

To use packages that are in development, it is necessary to have an `artifact_urls.yml` file located in the same path as the `wazuh-install-5.1.0.sh` script. This file must contain the URLs of the development packages that will be used in the installation. It must have the following format:
...
```bash
sudo bash wazuh-install-5.1.0.sh --all-in-one --development local
# or use the short form
sudo bash wazuh-install-5.1.0.sh -a -d local
```

This command will automatically detect the `artifact_urls.yml` file in the same path as the `wazuh-install-5.1.0.sh` script and will use the URLs specified in it to download the necessary packages for the installation.
...
For the certs-tool to detect the file, it must be located in the same path as the `wazuh-certs-tool-5.1.0.sh` script.
...
```bash
sudo bash wazuh-certs-tool-5.1.0.sh --all
# or use the short version
sudo bash wazuh-certs-tool-5.1.0.sh -A
```
...
```bash
sudo bash wazuh-certs-tool-5.1.0.sh -A </path/to/root-ca.pem> </path/to/root-ca.key>
```
...
- Create root CA:
    ...
    ```bash
    sudo bash wazuh-certs-tool-5.1.0.sh --root-ca-certificates
    # or use the short version
    sudo bash wazuh-certs-tool-5.1.0.sh -ca
    ```

- Create Wazuh indexer certificates:

    ```bash
    sudo bash wazuh-certs-tool-5.1.0.sh --wazuh-indexer-certificates </path/to/root-ca.pem> </path/to/root-ca.key>
    # or use the short version
    sudo bash wazuh-certs-tool-5.1.0.sh -wi </path/to/root-ca.pem> </path/to/root-ca.key>
    ```

- Create Wazuh manager certificates:

    ```bash
    sudo bash wazuh-certs-tool-5.1.0.sh --wazuh-manager-certificates </path/to/root-ca.pem> </path/to/root-ca.key>
    # or use the short version
    sudo bash wazuh-certs-tool-5.1.0.sh -wm </path/to/root-ca.pem> </path/to/root-ca.key>
    ```

- Create Wazuh dashboard certificates:

    ```bash
    sudo bash wazuh-certs-tool-5.1.0.sh --wazuh-dashboard-certificates </path/to/root-ca.pem> </path/to/root-ca.key>
    # or use the short version
    sudo bash wazuh-certs-tool-5.1.0.sh -wd </path/to/root-ca.pem> </path/to/root-ca.key>
    ```

- Create admin certificates:

    ```bash
    sudo bash wazuh-certs-tool-5.1.0.sh --admin-certificates </path/to/root-ca.pem> </path/to/root-ca.key>
    # or use the short version
    sudo bash wazuh-certs-tool-5.1.0.sh -a </path/to/root-ca.pem> </path/to/root-ca.key>
    ```
...
```bash
sudo ./wazuh-passwords-tool-5.1.0.sh -u <USER> [-p <PASSWORD>]
```
...
```bash
sudo ./wazuh-passwords-tool-5.1.0.sh -A -au <ADMIN_USER> -ap <ADMIN_PASSWORD> -u <USER> [-p <PASSWORD>]
```
...
```bash
sudo ./wazuh-passwords-tool-5.1.0.sh -A -au wazuh -ap wazuh -u wazuh -p N3wS3cr3tP4ss*
```
