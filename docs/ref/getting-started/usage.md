...
The Wazuh Installation Assistant is used by running the previously downloaded `wazuh-install-5.9.9-1.sh` script. Depending on the type of installation you want to perform (AIO or a specific component), the steps vary.
...
| `-d [pre-release\|local]`, `--development` | Use development repositories. By default it uses the pre-release package repository. If local is specified, it will use a local artifact_urls.yml file located in the same path as the wazuh-install-5.9.9-1.sh. |
...
```bash
sudo bash wazuh-install-5.9.9-1.sh --all-in-one
# or use the short form
sudo bash wazuh-install-5.9.9-1.sh -a
```
...
2. Create the necessary files for installation that will be stored in `wazuh-install-files.tar` with the following command:
    ...
    ```bash
    sudo bash wazuh-install-5.9.9-1.sh --generate-config-files
    # or use the short form
    sudo bash wazuh-install-5.9.9-1.sh -g
    ```
...
4. Once you have the `wazuh-install-files.tar` file on the machine where you will install the component, you just need to run the installation command for the desired component:
    ...
    ``` bash
    sudo bash wazuh-install-5.9.9-1.sh --wazuh-server
    # or use the short form
    sudo bash wazuh-install-5.9.9-1.sh -ws
    ```

    4.2 To install the Wazuh Indexer:

    ```bash
    sudo bash wazuh-install-5.9.9-1.sh --wazuh-indexer
    # or use the short form
    sudo bash wazuh-install-5.9.9-1.sh -wi
    ```

    4.3 To install the Wazuh Dashboard:

    ```bash
    sudo bash wazuh-install-5.9.9-1.sh --wazuh-dashboard
    # or use the short form
    sudo bash wazuh-install-5.9.9-1.sh -wd
    ```
...
1. On a system with Internet access, download the packages of the central components you want to install on the offline system. Note that you also need to have the wazuh-install-5.9.9-1.sh on the system with Internet access.
See the [Installation Assistant Installation](../../installation/installation-assistant/ia-installation.md) section to learn how to download `wazuh-install-5.9.9-1.sh`.

    To download the packages necessary for offline installation, run the following command:

    ```bash
    sudo bash wazuh-install-5.9.9-1.sh --download-wazuh <TYPE> --download-arch <ARCH>
    # or use the short form
    sudo bash wazuh-install-5.9.9-1.sh -dw <TYPE> -da <ARCH>
    ```
...
3. Transfer all files (`wazuh-offline.tar.gz`, `wazuh-install-files.tar` and `wazuh-install-5.9.9-1.sh`) to the offline system where you want to install Wazuh using your preferred method (USB, SCP, etc).

#### Perform the offline installation

Once you have the `wazuh-offline.tar.gz`, `wazuh-install-files.tar` and `wazuh-install-5.9.9-1.sh` files on the offline system, the installation is done the same way as a normal Wazuh installation (you can see how it's done in the `AIO Installation` and `Specific Component Installation` sections found above in this document), but by specifying the `-of, --offline-installation` option to the installation command.
For example, to perform an offline AIO installation, the command would be:

```bash
sudo bash wazuh-install-5.9.9-1.sh --all-in-one --offline-installation
# or use the short form
sudo bash wazuh-install-5.9.9-1.sh -a -of
```
...
```bash
sudo bash wazuh-install-5.9.9-1.sh --wazuh-server --offline-installation
# or use the short form
sudo bash wazuh-install-5.9.9-1.sh -ws -of
```
...
```bash
sudo bash wazuh-install-5.9.9-1.sh --all-in-one --development pre-release
# or use the short form
sudo bash wazuh-install-5.9.9-1.sh -a -d pre-release
```

#### Use development packages

To use packages that are in development, it is necessary to have an `artifact_urls.yml` file located in the same path as the `wazuh-install-5.9.9-1.sh` script. This file must contain the URLs of the development packages that will be used in the installation. It must have the following format:
...
```bash
sudo bash wazuh-install-5.9.9-1.sh --all-in-one --development local
# or use the short form
sudo bash wazuh-install-5.9.9-1.sh -a -d local
```

This command will automatically detect the `artifact_urls.yml` file in the same path as the `wazuh-install-5.9.9-1.sh` script and will use the URLs specified in it to download the necessary packages for the installation.

## Wazuh certs tool

The certs-tool is used by running the previously downloaded `wazuh-certs-tool-5.9.9-1.sh` script along with the `config.yml` configuration file. The certs tool generates the necessary certificates for the nodes specified in the configuration file.
...
For the certs-tool to detect the file, it must be located in the same path as the `wazuh-certs-tool-5.9.9-1.sh` script.
...
```bash
sudo bash wazuh-certs-tool-5.9.9-1.sh --all
# or use the short version
sudo bash wazuh-certs-tool-5.9.9-1.sh -A
```
...
```bash
sudo bash wazuh-certs-tool-5.9.9-1.sh -A </path/to/root-ca.pem> </path/to/root-ca.key>
```
...
- Create root CA:
    ...
    ```bash
    sudo bash wazuh-certs-tool-5.9.9-1.sh --root-ca-certificates
    # or use the short version
    sudo bash wazuh-certs-tool-5.9.9-1.sh -ca
    ```

- Create Wazuh indexer certificates:

    ```bash
    sudo bash wazuh-certs-tool-5.9.9-1.sh --wazuh-indexer-certificates </path/to/root-ca.pem> </path/to/root-ca.key>
    # or use the short version
    sudo bash wazuh-certs-tool-5.9.9-1.sh -wi </path/to/root-ca.pem> </path/to/root-ca.key>
    ```

- Create Wazuh server certificates:

    ```bash
    sudo bash wazuh-certs-tool-5.9.9-1.sh --wazuh-server-certificates </path/to/root-ca.pem> </path/to/root-ca.key>
    # or use the short version
    sudo bash wazuh-certs-tool-5.9.9-1.sh -ws </path/to/root-ca.pem> </path/to/root-ca.key>
    ```

- Create Wazuh dashboard certificates:

    ```bash
    sudo bash wazuh-certs-tool-5.9.9-1.sh --wazuh-dashboard-certificates </path/to/root-ca.pem> </path/to/root-ca.key>
    # or use the short version
    sudo bash wazuh-certs-tool-5.9.9-1.sh -wd </path/to/root-ca.pem> </path/to/root-ca.key>
    ```

- Create admin certificates:

    ```bash
    sudo bash wazuh-certs-tool-5.9.9-1.sh --admin-certificates </path/to/root-ca.pem> </path/to/root-ca.key>
    # or use the short version
    sudo bash wazuh-certs-tool-5.9.9-1.sh -a </path/to/root-ca.pem> </path/to/root-ca.key>
    ```
...
The `wazuh-passwords-tool-5.9.9-1.sh` script provides the following options for managing Wazuh internal user passwords:
...
```bash
sudo ./wazuh-passwords-tool-5.9.9-1.sh -u <USER> [-p <PASSWORD>]
```
...
```bash
sudo ./wazuh-passwords-tool-5.9.9-1.sh -u admin -p Secr3tP4ssw*rd
```
...
```bash
sudo ./wazuh-passwords-tool-5.9.9-1.sh -A -au <ADMIN_USER> -ap <ADMIN_PASSWORD> -u <USER> [-p <PASSWORD>]
```
...
```bash
sudo ./wazuh-passwords-tool-5.9.9-1.sh -A -au wazuh -ap wazuh -u wazuh -p N3wS3cr3tP4ss*
```
