...
| Options | Purpose |
|---------|---------|
| `-A\|--api` | Change the Wazuh manager API password given the current password. Requires `-u\|--user <USER>`, `-p\|--password <PASSWORD>`, `-au\|--admin-user <ADMIN_USER>`, and `-ap\|--admin-password <ADMIN_PASSWORD>`. |
| `-au\|--admin-user <ADMIN_USER>` | Admin user for the Wazuh manager API. Required for changing the Wazuh manager API passwords. Requires `-A\|--api`. |
| `-ap\|--admin-password <ADMIN_PASSWORD>` | Password for the Wazuh manager API admin user. Required for changing the Wazuh manager API passwords. Requires `-A\|--api`. |
...
There are two types of users whose passwords can be changed with this tool: Wazuh indexer users and Wazuh manager API users. For the latter, it is necessary to provide an administrator user and their password to authenticate the password change request.
...
## Change Wazuh manager API password

To change the password of a Wazuh manager API user, use the following syntax:

```bash
sudo ./wazuh-passwords-tool.sh -A -au <ADMIN_USER> -ap <ADMIN_PASSWORD> -u <USER> [-p <PASSWORD>] 
```

Where `<ADMIN_USER>` is the Wazuh manager API administrator user, `<ADMIN_PASSWORD>` is the administrator user's password, `<USER>` is the name of the user whose password you want to change, and `<PASSWORD>` is the new password. If `<PASSWORD>` is not specified, the tool will generate a random password.
