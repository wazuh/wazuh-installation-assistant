...
It is very important to ensure that the `config.yml` file is correctly configured with the name and IP address of each node. You can also add one or more DNS names per node using the `dns` field to include them in the generated certificate.
...
Each node must have a unique name and an associated IP address. Optionally, each node can include a `dns` list with one or more DNS names. In the case of Wazuh manager nodes, if there is more than one node, it is necessary to specify the node type (master or worker) using the `node_type` field.
