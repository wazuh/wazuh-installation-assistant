...
- `add last stage`: The installation assistant tool, when built for releases or pre-releases, needs to set a script variable called `last_stage` with the pre-release version it is being built with (alpha1, alpha0, rc1, etc). If set to true, the workflow will set this variable with the value provided in the input.
