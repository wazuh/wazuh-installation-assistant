...
```
  ...
  ```BASH
    bash wazuh-install-10.9.9.sh --generate-config-files
  ```
...
Follow these steps to install and configure a multi-node Wazuh indexer.
  ...
  ```bash
      bash wazuh-install-10.9.9.sh --wazuh-indexer indexer
  ```
...
```BASH
      bash wazuh-install-10.9.9.sh --start-cluster
```
...
### Wazuh manager cluster installation
  ...
  ```BASH
        bash wazuh-install-10.9.9.sh --wazuh-manager manager
  ```
...
### Wazuh dashboard installation
  ...
  ```BASH
        bash wazuh-install-10.9.9.sh --wazuh-dashboard dashboard
  ```
