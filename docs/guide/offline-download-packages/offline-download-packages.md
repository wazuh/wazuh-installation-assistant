...
```bash
curl -sO https://packages.wazuh.com/production/10.x/wazuh-install-10.9.9.sh
chmod 744 wazuh-install-10.9.9.sh
```
...
```bash
./wazuh-install-10.9.9.sh -dw rpm -da x86_64
```

##### aarch64

```bash
./wazuh-install-10.9.9.sh -dw rpm -da aarch64
```
...
```bash
./wazuh-install-10.9.9.sh -dw deb -da amd64
```

##### arm64

```bash
./wazuh-install-10.9.9.sh -dw deb -da arm64
```

### 3. Download the certificates configuration file

```bash
curl -s -o config.yml https://packages.wazuh.com/production/10.x/config-10.9.9.yml
```
