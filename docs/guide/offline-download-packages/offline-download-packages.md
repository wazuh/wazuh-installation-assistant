...
chmod 744 wazuh-install-5.0.1.sh
```
...
chmod 744 wazuh-install-5.0.1-<STAGE>.sh
```
...
```bash
./wazuh-install-5.0.1.sh -dw rpm -da x86_64
```

To install `pre-release` packages instead, use:

```bash
./wazuh-install-5.0.1-<STAGE>.sh -dw rpm -da x86_64 -d pre-release
```

##### aarch64

```bash
./wazuh-install-5.0.1.sh -dw rpm -da aarch64
```

To install `pre-release` packages instead, use:

```bash
./wazuh-install-5.0.1-<STAGE>.sh -dw rpm -da aarch64 -d pre-release
```
...
```bash
./wazuh-install-5.0.1.sh -dw deb -da amd64
```

To install `pre-release` packages instead, use:

```bash
./wazuh-install-5.0.1-<STAGE>.sh -dw deb -da amd64 -d pre-release`
```

##### arm64

```bash
./wazuh-install-5.0.1.sh -dw deb -da arm64
```

To install `pre-release` packages instead, use:

```bash
./wazuh-install-5.0.1-<STAGE>.sh -dw deb -da arm64 -d pre-release`
```
