...
```bash
curl -sO https://packages.wazuh.com/production/5.x/wazuh-install-5.9.9-1.sh
chmod 744 wazuh-install-5.9.9-1.sh
```
...
```bash
./wazuh-install-5.9.9-1.sh -dw rpm -da x86_64
```

##### aarch64

```bash
./wazuh-install-5.9.9-1.sh -dw rpm -da aarch64
```
...
```bash
./wazuh-install-5.9.9-1.sh -dw deb -da amd64
```

##### arm64

```bash
./wazuh-install-5.9.9-1.sh -dw deb -da arm64
```

### 3. Download the certificates configuration file

```bash
curl -s -o config.yml https://packages.wazuh.com/production/5.x/config-5.9.9-1.yml
```
