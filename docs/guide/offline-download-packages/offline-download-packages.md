...
chmod 744 wazuh-install-5.1.0.sh
```
...
chmod 744 wazuh-install-5.1.0-<STAGE>.sh
```
...
```bash
./wazuh-install-5.1.0.sh -dw rpm -da x86_64
```

To install `pre-release` packages instead, use:

```bash
./wazuh-install-5.1.0-<STAGE>.sh -dw rpm -da x86_64 -d pre-release
```

##### aarch64

```bash
./wazuh-install-5.1.0.sh -dw rpm -da aarch64
```

To install `pre-release` packages instead, use:

```bash
./wazuh-install-5.1.0-<STAGE>.sh -dw rpm -da aarch64 -d pre-release
```
...
```bash
./wazuh-install-5.1.0.sh -dw deb -da amd64
```
...
```bash
./wazuh-install-5.1.0.sh -dw deb -da arm64
```
